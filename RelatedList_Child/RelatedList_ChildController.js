({
    ...
    addRelatedRecord: function (cmp, evt, help) {
        ...
        let arrofRecs = [], Ids = cmp.get('v.selectedIds'), objName = cmp.get('v.pri_objectName'),
            relatedApiName = cmp.get('v.pri_relatedField'), recID = cmp.get('v.recordId');
        Ids.map(val => {
            let obj = {};
            obj['attributes'] = { 'type': objName };
            obj[relatedApiName] = recID;
            obj[cmp.get('v.sec_JunctionField')] = val;
            arrofRecs.push(obj);
        })
